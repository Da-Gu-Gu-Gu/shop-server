...
router.get('/getall', auth, async (req, res) => {
    try {
        ...
        await Order.find().populate("userId").populate({path:'products',populate:'productId'})
            .then((order) => {
                if (!order) return res.status(402).json("Error Occured")
...
router.get('/:id', auth, async (req, res) => {
    try {
        await Order.findOne({ userId: req.params.id }).populate("userId").populate({path:'products',populate:'productId'})
            .then((order) => {
                if (!order) return res.status(402).json("Error Occured")
                res.status(200).json(order)
            })
...
router.put('/:id', auth, async (req, res) => {
    try {
        ...
        await Order.findById(req.params.id)
            .then(async (order) => {
                if (!order) return res.status(402).json("order not found")
